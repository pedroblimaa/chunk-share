...
const chunkShareApi = {
  ...
  storage: {
    ...
    onProviderCopyProgress: (listener: (progress: StorageProviderCopyProgress) => void): (() => void) => {
      ...
      ipcRenderer.on(STORAGE_PROVIDER_COPY_PROGRESS_CHANNEL, progressListener)

      return () => ipcRenderer.removeListener(STORAGE_PROVIDER_COPY_PROGRESS_CHANNEL, progressListener)
    },
