  ...
  switch (scenario) {
    case StorageProviderSwitchScenario.BothHaveData:
      return {
        ...
        description: 'Choose which save ChunkShare should use.',
        activateLabel: `Use ${targetLabel} data (Recommended)`,
        copyLabel: `Replace ${targetLabel} with current save`
      }
    case StorageProviderSwitchScenario.SourceOnly:
      return {
        title: 'Only the current provider contains saves',
        description: `Copy the current save or activate an empty ${targetLabel}.`,
        activateLabel: `Activate empty ${targetLabel}`,
        copyLabel: `Copy save and activate ${targetLabel} (Recommended)`
      }
